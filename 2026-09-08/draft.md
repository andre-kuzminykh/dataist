...
Представьте задачу: вы даёте ИИ научную статью по машинному обучению и просите собрать по ней целый репозиторий. Не один файл, не игрушечную функцию, а нормальный проект: модель, загрузку данных, обучение, оценку, скрипты запуска. На бумаге всё звучит логично. На практике ИИ почти всегда начинает упрощать.

Где-то теряется важная деталь алгоритма. Где-то метрика считается «примерно так же». Где-то один файл ожидает тензор в одном формате, а другой выдаёт в другом. В итоге код выглядит правдоподобно, но метод из статьи уже другой.
...
[FIGURE:0]
[CAPTION:Три подхода к переводу статьи в код: обычные ИИ-агенты, специализированные пайплайны и схема PaperCompiler со спецификацией на уровне репозитория.]
...
[FIGURE:2]
[CAPTION:Общий пайплайн PaperCompiler: разбор статьи, сборка спецификации, распределение ответственности по файлам и генерация репозитория по зависимостям.]
...
[FIGURE:1]
[CAPTION:Кейс Universal Neural Functionals: обычный пайплайн схлопывает важное правило алгоритма, а PaperCompiler сохраняет перечисление допустимых разбиений.]
...
Результаты выглядят так: против главного сравнимого базового метода, PaperCoder, новый подход улучшает среднюю оценку:
...
[FIGURE:6]
[CAPTION:Доли побед по отдельным статьям на подмножестве из десяти работ: PaperCompiler чаще обходит AutoP2C, AutoReproduce и PaperCoder, особенно при сравнении с авторским кодом.]
...
PaperCompiler не решает задачу полностью. И авторы этого не скрывают.
...
**PaperCompiler** показывает простую вещь: чтобы переводить статьи в код точнее, мало дать модели больше контекста и больше токенов. Нужно **сначала собрать спецификацию на уровне репозитория**, а уже потом писать файлы.

Такой подход заметно снижает алгоритмические упрощения, уменьшает число тяжёлых ошибок и лучше сохраняет детали метода при сравнении с авторским кодом. Главный урок здесь шире самой задачи перевода статьи в код: в длинных инженерных сценариях прогресс даёт не только модель, но и то, **как пайплайн хранит, передаёт и защищает смысловые требования**.
