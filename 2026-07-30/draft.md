...
Это важно, потому что в бизнесе ошибка часто выглядит не как «модель не справилась». Ошибка выглядит иначе: агент все сделал аккуратно, уверенно, быстро — и нарушил внутреннюю политику компании.
...
[FIGURE:0]
[CAPTION:Статистика бенчмарка: длина регламентов, число критериев проверки и доля запрещённых действий по областям.]
...
Это важный сдвиг. Вместо «умеет ли модель быть полезной» исследователи спрашивают: **умеет ли она быть управляемой при длинной инструкции**.
...
Результаты вышли отрезвляющими — в хорошем смысле слова здесь нельзя использовать это слово, так что скажем прямо: результаты получились очень ясными.
...
[FIGURE:1]
[CAPTION:Строгий pass@1 в сравнении со средней стоимостью прогона и числом выходных токенов; пунктиром показана граница Парето.]
...
Для команд, которые строят ИИ-агентов, это практический сигнал. Оптимизировать надо не только качество «в среднем», но и тип ошибки. Если агент нарушает политику, рост бюджета на инференс может просто сделать это нарушение дороже.
...
[FIGURE:2]
[CAPTION:Сравнение строгой метрики и смягчённой, где допускается один проваленный критерий.]
...
HANDBOOK.md проверяет неприятную, но центральную вещь: способен ли ИИ-агент держаться длинного регламента на протяжении реальной многосоставной задачи. Пока ответ сдержанный.
...
<!-- [FIGURE:0] Figure 1: Dataset statistics across the 65 tasks: (a) handbook length in pages; (b) rubric criteria per task; (c) rubric criteria by domain, split into Expected-Output and Incorrect-Behavior types. -->
<!-- [FIGURE:1] Figure 2: Strict pass@1 against (a) mean cost per trial and (b) mean output tokens per trial, for the twenty configurations in the June 2026 release evaluation. Dashed lines trace the Pareto frontier. Cost and token values are per-trial means over all trials of a configuration. -->
<!-- [FIGURE:2] Figure 3: Strict pass@1 (circles) versus pass@1 ( N − 1 N{-}1 ) (diamonds), which tolerates one failed criterion per trial; June 2026 release configurations. -->
