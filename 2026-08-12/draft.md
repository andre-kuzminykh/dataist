...
Это важно, потому что рефакторинг в реальной разработке редко живёт в одном файле. Вы меняете интерфейс, переносите типы, переименовываете поля, выносите общую логику — и вслед за этим тянутся вызовы, тесты, конфиги, документация, обвязка сборки. У агента должна быть не только локальная аккуратность, но и длинное планирование.

[FIGURE:0]
[CAPTION:Пайплайн отбора и ручной доработки задач: от десятков тысяч коммитов до 170 проверенных примеров.]
...
[FIGURE:1]
[CAPTION:Матрица совместной встречаемости типов задач: рефакторинг часто идёт вместе с изменением API и исправлением ошибок.]
...
<!-- [FIGURE:0] Figure 3 : Data collection and curation pipeline for SWE-Bench ProMax. -->
<!-- [FIGURE:1] Figure 8: Co-occurrence matrix of task categories. API Interface Change and Refactoring Cleanup co-occur in 79 instances, while Bug Fix frequently accompanies both (33 and 48 instances respectively), reflecting how refactoring often surfaces latent defects that must be addressed concurrently. -->
