...
[FIGURE:0]
[CAPTION:Ассистент видит не саму социальную ситуацию , а её пересказ через пользователя с его памятью и предвзятостью.]
...
[FIGURE:1]
[CAPTION:Схема Fuse: от категорий ментальных состояний и шаблонов сценариев до симуляции , пользовательского пересказа и оценки ответа модели.]
...
[FIGURE:3]
[CAPTION:Разбивка результатов моделей: правильные ответы , ошибки и отказы от ответа в задаче социального рассуждения через пользователя.]
...
[FIGURE:4]
[CAPTION:Сравнение двух режимов: когда модель видит сами события и когда слышит о них только через пересказ пользователя.]
...
[FIGURE:5]
[CAPTION:Когда пользователь подаёт ту же историю с предвзятым уклоном , качество падает у всех моделей сильнее , чем у людей.]
...
[FIGURE:6]
[CAPTION:С ростом подробности рассказа модели улучшаются быстрее , чем люди , но всё равно не догоняют человеческий уровень.]
...
Интуитивно кажется , что многоходовый разговор должен решать проблему. Если деталей не хватает , модель спросит ещё. Пользователь ответит. Качество вырастет.
...
<!-- [FIGURE:0] Figure 1 : AI assistants operate within a user-mediated reality : the assistant hears about social events through the subjective lens of the user , whose account may be partial or biased, making it challenging to reason about the true motive of an actor in the user’s circle. -->
<!-- [FIGURE:1] Figure 2 : Overview of the evaluation framework. (1) Mental-state categories are converted into (2) scenario templates and (3) instantiated via multi-agent simulation with a latent ground-truth motive. (4) The simulated user debriefs the evaluated assistant using their subjective memories, and (5) a judge LLM evaluates the assistant’s motive prediction against the ground truth. -->
...
<!-- [FIGURE:3] Figure 4 : Performance and behavioral breakdown on the user-mediated social reasoning task. The stacked bars illustrate the proportion of decisions that resulted in a Correct , Incorrect , or Not Attempted outcome. The MSR score from Equation 1 is displayed near each bar. -->
<!-- [FIGURE:4] Figure 5 : Performance in objective ( Observer ) vs. user-mediated ( Assistant ) settings, highlighting two challenges in social reasoning.
(i) Some models exhibit noticeable error rates even when presented with objective events. (ii) Relying on subjective user retellings causes higher error rates and higher abstention rates. -->
<!-- [FIGURE:5] Figure 6 : Performance under default and biased user framing for all evaluated models (sorted by the gap between conditions) alongside the human majority baseline (our proxy for task solvability).
Bias degrades performance across all models. The average model gap (7.7) is more than double the drop in human baseline (3.6), suggesting that the degradation is not driven by information loss alone. -->
<!-- [FIGURE:6] Figure 8 : MSR across narrative detail levels for all models , their mean , and the human majority baseline (our proxy for task solvability). With more detail, model MSR rises faster than solvability, narrowing the gap by 22% (from 14.1 to 11.0). -->
<!-- [FIGURE:7] Figure 10 : MSR across dialogue turns. Performance at turn 4 as users provide clarifying evidence, then plateaus or degrades due to ineffective clarification and compounding bias. Even with extended turns, no model approaches the human baseline at turn 2 or matches their own high-detail baseline. -->
<!-- [FIGURE:8] Figure 15 : Truncated example of one of the transcripts that were sent to human raters during the main simulation validation study. A similar format was used for LLMs during the observer task (§ 4.1 ). -->
<!-- [FIGURE:9] Figure 16 : Full Observer vs. Assistant results for all evaluated models, expanding Figure 5 in § 4.1 . -->
