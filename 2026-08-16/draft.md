...
Авторы называют подход «сильный-к-слабому через каркас». По сути это такой сценарий:
...
🟠 на этих 5% он итеративно улучшает каркас: промты, маршрутизацию, правила, код, проверки, формат ответа

🟠 потом готовый каркас тестируют на скрытом полном наборе

[FIGURE:0]
[CAPTION:Схема эксперимента: сильная модель итеративно строит каркас по маленькой валидации, а потом его проверяют на скрытом тесте.]
...
Самая слабая цель в основном эксперименте — GPT-5.4-mini. Без каркаса она набирала в среднем 0.488. С каркасом средний результат по всем запускам вырос до 0.763. Лучший запуск добрался до 0.912.

[FIGURE:2]
[CAPTION:Средняя точность связок «модель-строитель × платформа»: почти все варианты заметно выше базового вызова слабой модели без каркаса.]

Коротко по цифрам:

🟣 базовый вызов GPT-5.4-mini без каркаса: 0.488

🟣 средний результат всех каркасов: 0.763
...
Иногда дешевле и полезнее не брать модель классом выше, а окружить текущую модель правильным пайплайном. В работе есть и более сильное наблюдение: хорошо собранный каркас для GPT-5.4-mini местами обгоняет более сильные модели без каркаса.

Но есть и ограничение. Человеческий, вручную придуманный каркас всё ещё лучше: он дал 0.939. Значит, автоматическая сборка уже работает хорошо, но до человеческой инженерии на сложных подзадачах ещё не всегда дотягивает.
...
В-третьих, много даёт жёсткий контроль формата ответа. Это инженерная деталь, но слабые модели часто теряют очки на банальных вещах: не тот формат, лишний текст, неправильный выбор опции. Каркас это обрезает.

[FIGURE:4]
[CAPTION:Разные сильные каркасы исправляют разные ошибки базовой модели: вместе они покрывают почти все типы промахов.]
...
Если упростить до одной фразы: лучшие каркасы не «мотивируют» слабую модель думать усерднее, а снимают с неё лишнюю умственную нагрузку.
...
Все строители работали только с маленькой валидацией. В среднем делали около пяти прогонов на ней. И здесь ещё один вывод: больше прогонов по валидации почти не помогает. Качество итогового каркаса слабо связано с количеством таких итераций. Зато очень сильно связано с качеством самой модели-строителя.
...
## Сильнее строитель — лучше каркас

Одна из самых чистых частей статьи — сравнение уровней «усилия рассуждения» у одной и той же модели-строителя, Opus-4.7. Ей давали разный бюджет на внутреннее рассуждение при написании каркаса: низкий, средний, высокий и очень высокий.

Результат почти монотонный: чем больше усилие, тем лучше каркас.

[FIGURE:3]
[CAPTION:Статистическая значимость лучшего каркаса против базовой линии: исправленных ошибок намного больше, чем новых промахов.]

Это интересно по двум причинам.

Первая: вычисления на этапе построения каркаса окупаются. То есть полезно не только «думать во время ответа», но и «думать во время проектирования пайплайна».
...
🟠 больше усилия у строителя → лучше итоговый каркас
...
## Кому каркасы помогают больше всего

Самый большой выигрыш получают слабые модели с заметным запасом до потолка. Когда целевая модель и так уже сильная, каркас помогает меньше. Иногда даже мешает.
...
Логика тут простая: каркас в первую очередь возвращает модели те способности, которые у неё уже как будто есть, но она применяет их нестабильно. Если модель и так справляется, лишние правила и маршруты начинают шуметь.

Вывод для практики:

🟣 каркасы особенно полезны для слабых и дешёвых моделей
...
Лучше всего автоматические каркасы справились с задачами, где есть явная структура: кто что видел, кто не видел, какой вывод отсюда следует. Там детерминированные правила работают почти идеально.
...
То есть каркас отлично снимает рутинную часть когнитивной нагрузки, но не заменяет саму способность к сложному рассуждению там, где задача остаётся по-настоящему открытой.
...
🟠 сильная модель может заметно улучшить слабую без дообучения, только за счёт каркаса на этапе инференса
...
<!-- [FIGURE:0] Figure 1 : Overview of the Strong-to-Weak Scaffolding evaluation framework. During recursive scaffold building , the builder model iteratively refines a scaffold to improve target-model performance on validation sets. During hidden evaluation testing , the final scaffold is evaluated on the full hidden test set to measure target-model performance. -->
<!-- [FIGURE:1] (a) Builder × \times platform mean accuracy. -->
<!-- [FIGURE:2] (a) Builder × \times platform mean accuracy. -->
<!-- [FIGURE:3] (a) Statistical significance of the best scaffold compared with the baseline. -->
<!-- [FIGURE:4] (c) Complementarity among repaired errors. -->
